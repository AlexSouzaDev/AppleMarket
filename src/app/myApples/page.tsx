...
export default function Store() {
    ...
    const formatModel = (id: string) => id.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
        ...
        <div className="min-h-screen bg-black text-white">
            ...
            <main className="mx-auto max-w-6xl px-4 sm:px-6 pt-10 pb-24">
                <div className="mb-10 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <h1 className={`${headingFont.className} text-4xl md:text-5xl font-black tracking-tight`}>My Devices</h1>
                        <p className="mt-2 text-sm text-white/50">A lightweight inventory of every iPhone you've valued.</p>
                    </div>
                    {isSignedIn && (
                        <div className="flex items-center gap-3">
                            <div className="relative">
                                ...
                                />
                                <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-white/40 text-xs">⌕</span>
                            </div>
                        </div>
                    )}
                </div>
                {isSignedIn ? (
                    <>
                        <div className="mb-6 flex flex-wrap items-center gap-4">
                            ...
                            <div className="ml-auto text-sm text-white/40">{user?.firstName || user?.primaryEmailAddress?.emailAddress}</div>
                        </div>

                        <div className="grid gap-4">
                            {filtered.map(d => {
                                const isEditing = editingId === d.id;
                                const prog = Math.min(100, Math.max(0, d.batteryHealth));
                                const ring = {
                                    background: `conic-gradient(var(--accent) ${prog}%, rgba(255,255,255,0.08) ${prog}% 100%)`
                                ...
                                return (
                                    <div
                                        ...
                                        <div className="flex items-start gap-5">
                                            <div className="relative">
                                                <div className={`h-16 w-16 rounded-2xl bg-gradient-to-br ${batteryColor(d.batteryHealth)} p-[2px]`}>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex flex-wrap items-start gap-x-3 gap-y-1">
                                                    <h3 className="text-base font-semibold leading-tight tracking-tight">
                                                        {formatModel(d.modelId)}
                                                    </h3>
                                                    <span className="rounded-full bg-white/5 px-2.5 py-0.5 text-[10px] uppercase tracking-wide text-white/50 border border-white/10">{d.condition}</span>
                                                    <span className="rounded-full bg-white/5 px-2 py-0.5 text-[10px] text-white/50 border border-white/10">Battery {d.batteryHealth}%</span>
                                                    <span className="rounded-full bg-emerald-400/10 text-emerald-300 border border-emerald-400/30 px-2 py-0.5 text-[10px]">€{d.estimatedPrice}</span>
                                                </div>
                                                {!isEditing && (
                                                ...
                                                {isEditing && (
                                                    <div className="mt-3 space-y-3">
                                                        ...
                                                        <div className="flex gap-2">
                                                            ...
                                                            >{noteMutation.isLoading ? 'Saving…' : 'Save'}</button>
                                                            <button
                                                                onClick={cancelEdit}
                                                ...
                                                <div className="mt-4 flex flex-wrap gap-3 text-xs">
                                                    ...
                                                    >{deleteMutation.isLoading ? '...' : 'Delete'}</button>
                                                    <span className="ml-auto self-center text-[10px] text-white/30">
                                                        {new Date(d.createdAt as any).toLocaleDateString()}
                                                    </span>
                                                </div>
                                            </div>
                                            <div className="hidden sm:flex h-16 w-16 rounded-full items-center justify-center border border-white/10 bg-black/40" style={ring}>
                            ...
                            {filtered.length === 0 && (
                                <div className="rounded-3xl border border-white/10 bg-white/5 p-10 text-center text-sm text-white/40 backdrop-blur-md">
                                    {devices && devices.length > 0 ? 'No matches for your search.' : 'No devices saved yet. Use the valuation form to add one.'}
                                </div>
                            )}
                        </div>
                    </>
                ) : (
                    <div className="mt-12 rounded-3xl border border-white/10 bg-gradient-to-b from-white/10 to-white/5 p-10 text-center backdrop-blur-xl">
